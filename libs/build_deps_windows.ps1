...
function Import-VsDevCmdEnv {
    ...
    # Use call command to properly invoke batch file and capture environment variables
    # This matches the approach used in build_windows.py
    # Use a single string argument to ensure proper quoting for paths with spaces
    $cmdLine = "call `"$VsDevCmd`" -no_logo -arch=amd64 -host_arch=amd64 && set"
    $EnvOutput = & cmd.exe /c $cmdLine 2>&1
    foreach ($line in $EnvOutput) {
        if ($line -match "^(.*?)=(.*)$") {
            $key = $matches[1]
            $value = $matches[2]
            # Skip empty keys and special variables that shouldn't be overwritten
            if ($key -and $key -ne "" -and $key -notmatch "^_") {
