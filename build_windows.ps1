<#
  ...
  Behavior:
    - No CLI flags; always builds Release with Qt6.
...
if (-not (Test-Path -LiteralPath $QtRoot)) {
    Write-ErrorMsg "QtRoot '$QtRoot' does not exist. Please place the Qt SDK there (qt_lib\\qt650)."
    exit 1
}
...
$DepsBuildDir = Join-Path $RepoRoot "libs\deps"
$DepsRoot = Join-Path $DepsBuildDir "built"

New-Item -ItemType Directory -Force -Path $DepsRoot | Out-Null
Write-Info "Using DepsRoot: $DepsRoot"

# Ensure third-party deps (yaml-cpp, zxing-cpp) are cached
$BuildDepsScript = Join-Path $RepoRoot "libs\build_deps_windows.ps1"
if (-not (Test-Path -LiteralPath $BuildDepsScript)) {
    Write-ErrorMsg "build_deps_windows.ps1 not found under libs/."
    exit 1
}
Write-Info "Ensuring cached third-party deps (yaml-cpp, zxing-cpp)..."
$Ok = Invoke-ProcessWithTimeout -FilePath "powershell" -ArgumentList @("-ExecutionPolicy", "Bypass", "-File", $BuildDepsScript, "-BuildDir", $DepsBuildDir, "-InstallPrefix", "built") -TimeoutSec $DepsBuildTimeoutSec
if (-not $Ok) {
    Write-ErrorMsg "Dependency build failed or timed out after ${DepsBuildTimeoutSec}s."
    exit 1
}
...
if (-not (Test-AnyPath -Paths $YamlConfigs)) {
    Write-ErrorMsg "yaml-cpp not found in '$DepsRoot'."
    exit 1
}
if (-not (Test-AnyPath -Paths $ZxingConfigs)) {
    Write-ErrorMsg "ZXing not found in '$DepsRoot'."
    exit 1
}

# Ensure Protobuf/gRPC deps are cached
Write-Info "Ensuring protobuf (for gRPC) is cached..."
$ProtobufScript = Join-Path $RepoRoot "libs\build_protobuf_windows.ps1"
if (-not (Test-Path -LiteralPath $ProtobufScript)) {
    Write-ErrorMsg "build_protobuf_windows.ps1 not found under libs/."
    exit 1
}
$Ok = Invoke-ProcessWithTimeout -FilePath "powershell" -ArgumentList @("-ExecutionPolicy", "Bypass", "-File", $ProtobufScript, "-BuildDir", $DepsBuildDir, "-InstallPrefix", "built") -TimeoutSec $DepsBuildTimeoutSec
if (-not $Ok) {
    Write-ErrorMsg "Protobuf build failed or timed out after ${DepsBuildTimeoutSec}s."
    exit 1
}
if (-not (Test-ProtobufInstall -InstallDir $DepsRoot)) {
    Write-ErrorMsg "Protobuf not found in '$DepsRoot' after build."
    exit 1
}
